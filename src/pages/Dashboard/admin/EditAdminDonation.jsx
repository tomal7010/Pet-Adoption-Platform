...
const EditAdminDonation = () => {
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        const res = await axiosSecure.get(`/api/admindonation-campaigns/${id}`);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      const res = await axiosSecure.patch(`/api/admindonation-campaigns/${id}`, updatedData);

      if (res.data?.modifiedCount > 0) {
        Swal.fire('Success', 'Campaign updated successfully', 'success');
        navigate('/dashboard/admin-all-donations');
      } else {
...
export default EditAdminDonation;
